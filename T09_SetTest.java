 ...
 */
	...
	public static void main(String[] args) {
		...
		// 그래서 이미 있는 데이터를 add하면 false를 반환하고, 데이터는
		// 추가되지 않는다.
		...
		// 불러올 수 없다. 그래서 데이터를 하나씩 얻기 위해서는 Iterator로
		// 변환해야 한다.
		// Set 데이터를 Iterator로 변환하기 => Set의 iterator()메서드를
		// 호출하면 된다.
		Iterator it = hs1.iterator();
		
		// 데이터 개수만큼 반복하기
		// hasNext()메서드 => 포인터 다음위치에 데이터가 있으면 true, 없으면
		// false를 반환한다.
		while(it.hasNext()) {  //다음 자료가 있는지 검사
			// next()메서드 => 포인터를 다음 자료 위치로 이동하고, 이동한
			// 				 위치의 자료를 반환한다.
		...
		// Collect유형의 객체들은 서로 다른 자료 구조로 쉽게 변경해서 사용할 수 있다.
